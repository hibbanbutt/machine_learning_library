...
def find_best_attribute(S, sample_size, attributes):
    if sample_size <= len(attributes):
        random_attributes = random.sample(attributes, sample_size)
    else:
        random_attributes = attributes

    best_gain = -1
    best_attribute = next(iter(random_attributes))
    ...
    if all(map(lambda x: x == S["y"][0], S["y"])):
        ...
        for value in values:
            ...
            if len(s_v) == 0:
                ...
                subtree = id3(s_v, attributes - {A}, sample_size)
    ...
    for k in {2, 4, 6}:
        ...
        for t in range(T):
            ...
            trees.append(id3(data, attributes={"age", "job", "marital", "education", "default",
                                               ...
                                               "duration", "campaign",
                                               "pdays", "previous", "poutcome"}, sample_size=k))
        ...
        plt.legend()
        plt.title(f"Random Forest of Size {k}")
        plt.show()
